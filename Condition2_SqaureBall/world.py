...
from game import Ball, Paddle, MovingBlock ,X, Y, BASE_DISTANCE_VALUE, BLOCK_LENGTH, BLOCK_WIDTH, BALL_SIZE

AGENT, OPPONENT, BALL, UPPER_BLOCK, LOWER_BLOCK = 0, 1, 2, 3, 4
WINDOW_WIDTH, WINDOW_HEIGHT = 800, 250
...
class World:
    def __init__(self, args):
        ...
        # Moving Blocks
        self.upper_block=MovingBlock(BASE_DISTANCE_VALUE)
        self.lower_block=MovingBlock(4*BASE_DISTANCE_VALUE)

    def handle_ball_movement(self, cx, cy, ublockx, lblockx):
        ...
        touching_p1 = nx <= 1
        touching_p2 = nx +BALL_SIZE +2.220446049250313e-10 >= self.canvas_width - 1  # -1 paddle -1 left
        ...
        if in_p2_area and touching_p2:
            nx = self.canvas_width - BALL_SIZE -1
            self.ball.direction[X] *= -1

        # check walls 1 = WALL THICKNESS
        hit_top = ny <= 1
        hit_bottom = ny +BALL_SIZE >= self.canvas_height

        if hit_top:
            ny = 1
            self.ball.direction[Y] *= -1
        if hit_bottom:
            ny = self.canvas_height -BALL_SIZE - 2.220446049250313e-10
            self.ball.direction[Y] *= -1
        ...
        # Y dir
        in_upper_block_area = self.ball.in_block_area(self.upper_block)
        in_lower_block_area= self.ball.in_block_area(self.lower_block)

        ## touching_upper_block_from_above=False
        ## touching_lower_block_from_above=False
        ## touching_upper_block_from_below=False
        ## touching_lower_block_from_below=False

        touching_upper_block_from_above=(BASE_DISTANCE_VALUE+5>= ny + BALL_SIZE >=BASE_DISTANCE_VALUE)
        touching_lower_block_from_above=(4*BASE_DISTANCE_VALUE+5>=ny + BALL_SIZE >= 4*BASE_DISTANCE_VALUE)
        touching_upper_block_from_below=(BASE_DISTANCE_VALUE<=ny <= BASE_DISTANCE_VALUE+5)
        touching_lower_block_from_below=(4*BASE_DISTANCE_VALUE<=ny <= 4* BASE_DISTANCE_VALUE+5)


        if (in_upper_block_area and (touching_upper_block_from_above or touching_upper_block_from_below)):
            if touching_upper_block_from_above:
                ny= BASE_DISTANCE_VALUE- 2.220446049250313e-10
                self.ball.direction[Y] *= -1
            else:
                ny=BASE_DISTANCE_VALUE + BLOCK_WIDTH + 2.220446049250313e-10
                self.ball.direction[Y] *= -1
        
        if (in_lower_block_area and (touching_lower_block_from_above or touching_lower_block_from_below)):
            if touching_lower_block_from_above:
                ny= 4* BASE_DISTANCE_VALUE+BLOCK_WIDTH - 2.220446049250313e-10
                self.ball.direction[Y] *= -1
        ...
        #ball moving to right side
        if ((ny<=BASE_DISTANCE_VALUE<=ny+BALL_SIZE) and (ublockx+BALL_SIZE>=nx>=ublockx)):
            self.ball.direction[X] *= -1
            nx=ublockx- 2.220446049250313e-10
        if ((ny<=4*BASE_DISTANCE_VALUE<=ny+BALL_SIZE) and (lblockx+BALL_SIZE>=nx>=lblockx)):
            self.ball.direction[X] *= -1
            nx= lblockx - 2.220446049250313e-10

        # ball moving to left side # 5 values are for standard 
        if ((ny<=BASE_DISTANCE_VALUE<=ny+BALL_SIZE) and (ublockx+BLOCK_LENGTH - 5 <=nx<=ublockx+BLOCK_LENGTH)):
            self.ball.direction[X] *= -1
            nx=ublockx + 2.220446049250313e-10
        if ((ny<=4*BASE_DISTANCE_VALUE<=ny+BALL_SIZE) and (lblockx+BLOCK_LENGTH - 5 <=nx<=lblockx+BLOCK_LENGTH)):
            self.ball.direction[X] *= -1
            nx= lblockx + 2.220446049250313e-10       
    ...
    def handle_Upper_block_movement(self):
        direction= self.upper_block.direction
        nx=self.upper_block.rect.x+ direction* MovingBlock.VELOCITY
        
        if (nx >=13*BASE_DISTANCE_VALUE-BLOCK_LENGTH):
            # block.x_vel *= -1
            self.upper_block.shift_back()
            nx=13*BASE_DISTANCE_VALUE-BLOCK_LENGTH - 2.220446049250313e-10
        elif(nx <=5*BASE_DISTANCE_VALUE):
            # block.x_vel *=-1
            self.upper_block.shift_back()
            nx= 5*BASE_DISTANCE_VALUE + 2.220446049250313e-10
        
        # nx= max(nx, 5*BASE_DISTANCE_VALUE)
        
        # nx=min(nx,13*BASE_DISTANCE_VALUE-BLOCK_LENGTH)
        
        return nx
    def handle_Lower_block_movement(self):
        direction= self.lower_block.direction
        nx=self.lower_block.rect.x+ direction* MovingBlock.VELOCITY
        
        if (nx >=13*BASE_DISTANCE_VALUE-BLOCK_LENGTH):
            # block.x_vel *= -1
            self.lower_block.shift_back()
            nx=13*BASE_DISTANCE_VALUE-BLOCK_LENGTH - 2.220446049250313e-10
        elif(nx <=5*BASE_DISTANCE_VALUE):
            # block.x_vel *=-1
            self.lower_block.shift_back()
    ...
    def game_reset(self):
        ...
        self.upper_block.reset()
        self.lower_block.reset()
    ...
    def get_state(self, initial=False):
        ...
        return (
            ...
            self.upper_block.get_position(),
            self.lower_block.get_position(),
        )

    @staticmethod
    def make_q_state(state):
        return state[AGENT], state[BALL], state[UPPER_BLOCK], state[LOWER_BLOCK]
    ...
    def apply_actions(self, state, action_agent, action_opponent, num_iter):
        ...
        # move ball
        bx, by = state[BALL]
        previous_upper_block_x=self.upper_block.get_position()
        previous_lower_block_x=self.lower_block.get_position()
        nx, ny = self.handle_ball_movement(bx, by, previous_upper_block_x, previous_lower_block_x)
        next_ball = (nx, ny)
        ...
        upper_block_x= self.handle_Upper_block_movement()
        lower_block_x=self.handle_Lower_block_movement()

        self.paddles[AGENT].set_position(agent_y)
        self.paddles[OPPONENT].set_position(opponent_y)
        self.upper_block.set_position(upper_block_x)
        self.lower_block.set_position(lower_block_x)


        next_state = (agent_y, opponent_y, next_ball, upper_block_x, lower_block_x)
    ...
    def final_show(self, Q):
        ...
        while not self.is_final_state(state, num_iter):
            ...
            self.upper_block.draw(self.canvas)
            self.lower_block.draw(self.canvas)
