...
class World:
    ...
    def handle_ball_movement(self, cx, cy):
        ...
        touching_p1 = nx <= 1
        touching_p2 = nx +30 +2.220446049250313e-10 >= self.canvas_width - 1  # -1 paddle -1 left
        ...
        if in_p2_area and touching_p2:
            nx = self.canvas_width - 30 -1
            self.ball.direction[X] *= -1

        # check walls 1 = WALL THICKNESS
        hit_top = ny <= 1
        hit_bottom = ny +30 >= self.canvas_height

        if hit_top:
            ny = 1
            self.ball.direction[Y] *= -1
        if hit_bottom:
            ny = self.canvas_height -30 - 2.220446049250313e-10
            self.ball.direction[Y] *= -1
